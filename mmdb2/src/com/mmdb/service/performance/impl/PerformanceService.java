...
//		long eTime = 1459661912000L;
	...
	public Map<Integer, List<PerformanceBean>> getPerformaceDatas(
		...
		try{
			...
			DBCursor rs = null;
			String tableName = "temp"+username;
			if(start > 0){
				total = (int) db.getCollection(tableName).count();
				rs = db.getCollection(tableName).find().sort(orderBy).limit(limit).skip(start);
			}else{
				MapReduceOutput out = col.mapReduce(map,  
		                reduce, tableName, query);  
				total = out.getOutputCount();
				Thread.sleep(200);
				rs = out.getOutputCollection().find().sort(orderBy).limit(limit).skip(start);
			}
